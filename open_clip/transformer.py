...
class ResidualAttentionBlock_Mul(nn.Module):
    def __init__(
            self,
            d_model: int,
            n_head: int,
...
class Transformer_Mul(nn.Module):
    def __init__(
            self,
            width: int,
            layers: int,
    ...
    def forward(self, x: torch.Tensor, out_layers: list = [3, 6, 9, 11], attn_mask: Optional[torch.Tensor] = None):
        idx = 0
        out_attn = []
        # out_tokens = x
        out_tokens = []
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.output_tokens:
            return pooled, tokens, Fp

        return pooled


class VisionTransformer_Mul(nn.Module):
    output_tokens: torch.jit.Final[bool]
    ...
    def __init__(
        ...
        self.ln_pre = norm_layer(width)
        self.transformer = Transformer_Mul(
            width,
            layers,
            heads,
            mlp_ratio,
...
class WindowVisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        img = torch.cat(
            [self.class_embedding.to(img.dtype) + torch.zeros(img.shape[0], 1, img.shape[-1], dtype=img.dtype, device=img.device),
             img], dim=1)
        img = img + self.positional_embedding.to(img.dtype)

        img = self.patch_dropout(img)
        img = self.ln_pre(img)
        ...
        if self.attn_pool is not None:
            img_feat = self.attn_pool(F_p)
            img_feat = self.ln_post(img_feat)
            img_feat, _ = self._global_pool(img_feat)
        else:
            img_feat, _ = self._global_pool(F_p)
            img_feat = self.ln_post(img_feat)

        if self.proj is not None:
            img_feat = img_feat @ self.proj
        ...
        if self.use_pix_multi_scale == True:
            ...
            for scale_index in range(len(self.scale_begin_indx)):
                ...
                mx = torch.cat(
                    [self.class_embedding.to(mx.dtype) + torch.zeros(mx.shape[0], 1, mx.shape[-1], dtype=mx.dtype, device=mx.device),
                     mx], dim=1)  # shape = [*, grid ** 2 + 1, width]
                mx = mx + self.positional_embedding.to(mx.dtype)[0, :]
                ...
                F_w += [p for p in pooled]

        else:
            raise NotImplementedError

        if self.output_tokens:
            return F_w, F_p, img_feat

        return F_w


class VisionTransformer_AD(nn.Module):
    output_tokens: torch.jit.Final[bool]
